...
>(
  (
    ...
    const isMobile = false; // simplified — no useIsMobile hook dependency
  ...
  ({ className, ...props }, ref) => {
    return (
      <input
        ...
        data-sidebar="input"
        className={cn(
          'flex h-8 w-full rounded-md bg-background px-2 py-1 text-sm shadow-none outline-none ring-ring transition-shadow focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground',
          className
        )}
        {...props}
      />
...
>(({ asChild: _asChild = false, isActive = false, tooltip, className, ...props }, ref) => {
...
>(({ asChild: _asChild = false, size = 'md', isActive, className, ...props }, ref) => {
  return (
    <a
      ref={ref}
