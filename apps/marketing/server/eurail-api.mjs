// @ts-check
// The Eurail onboarder API — ONE implementation, two homes:
//   • dev:  astro.config.mjs mounts these handlers on the Vite dev server (`astro dev`), so the
//           feature works on localhost with zero extra processes.
//   • prod: server/index.mjs wraps the same handlers in a tiny node:http service (the "sidecar")
//           behind an nginx `location /eurail/api/` proxy, next to the static site.
// Endpoints (connect-style mounting; req.url is the path AFTER the mount prefix):
//   POST /eurail/api/chat                  — gated, streams the grounded onboarder reply
//   GET  /eurail/api/chat/sessions[/<id>]  — gated, per-user conversation history
//   POST /eurail/api/auth/request|verify   — emailed 6-digit code → HttpOnly session cookie
//   GET  /eurail/api/auth/me · POST /eurail/api/auth/logout
// Design rule: NOTHING here reads its own import.meta.url — every path (corpus dir, sessions dir)
// is injected by the caller, so the module behaves identically when Astro bundles the dev config
// and when node runs the sidecar from /opt on a VPS.
...
/**
 * @typedef {Object} EurailApiOptions
 * @property {(name: string, fallback: string) => string} env   config lookup (process.env + .env file)
 * @property {URL} eurailDir    the app's src/eurail/ (corpus JSON, prompts, the diagram verifier)
 * @property {URL} sessionsDir  writable per-user conversation store (outside any --delete rsync)
 * @property {boolean} [secureCookies]  add `Secure` to the session cookie (behind HTTPS) — default false
 * @property {boolean} [logCodes]       echo issued codes to the console (dev convenience) — default false
 */

/**
 * Build the three connect-style handlers and mount them. `use` is connect/Vite's
 * `middlewares.use(prefix, handler)` in dev, or the sidecar's mini-router in prod — both strip the
 * mount prefix from req.url before the handler runs, which is the only contract assumed here.
 * @param {(prefix: string, handler: (req: any, res: any, next: () => void) => void | Promise<void>) => void} use
 * @param {EurailApiOptions} opts
 */
export function mountEurailApi(use, opts) {
  const { env, eurailDir, sessionsDir } = opts;
  const SECURE_COOKIES = opts.secureCookies === true;
  const LOG_CODES = opts.logCodes === true;

  const LLM_URL = env('EURAIL_ONBOARDER_URL', 'https://openrouter.ai/api/v1/chat/completions');
  const LLM_MODEL = env('EURAIL_ONBOARDER_MODEL', 'anthropic/claude-sonnet-4.6');
  const LLM_API_KEY = env('EURAIL_ONBOARDER_API_KEY', '');
  ...
  const CARTOGRAPHER_TIMEOUT_MS = 25_000;
  ...
  const safeId = (/** @type {unknown} */ s) => typeof s === 'string' && /^[a-z0-9-]{6,40}$/.test(s);

  // ── EMAIL GATE (@eurail.com) ─────────────────────────────────────────────────────────
  ...
  // session credential). In production set EURAIL_AUTH_SECRET to a stable value — otherwise a
  // fresh random secret is generated per boot and every session resets on restart.
  ...
  const AUTH_SECRET = env('EURAIL_AUTH_SECRET', '') || randomBytes(32).toString('hex');
  const EMAIL_API_KEY = env('EURAIL_EMAIL_API_KEY', '');
  const EMAIL_FROM = env('EURAIL_EMAIL_FROM', 'support@oraclous.com');
  ...
  const HAS_EMAIL_SERVICE = Boolean((SMTP_HOST && SMTP_USER && SMTP_PASS) || EMAIL_API_KEY);
  ...
  // Allowed logins: any @eurail.com address, plus admin test addresses (lets Reza receive a real
  // code at an inbox he owns, since @eurail.com inboxes aren't his). Remove before any real use.
  ...
  const MAX_PENDING_CODES = 1000; // bound the pending-code store
  const emailHash = (/** @type {string} */ e) =>
    createHash('sha256').update(normEmail(e)).digest('hex').slice(0, 24);
  ...
  const signToken = (/** @type {string} */ email) => {
    const body = Buffer.from(
      JSON.stringify({ e: normEmail(email), x: Date.now() + SESSION_TTL_MS }),
    ).toString('base64url');
  ...
  // `Secure` only behind HTTPS (prod) — http://localhost would refuse to store it in dev.
  const cookieTail = `HttpOnly; SameSite=Strict; Path=/${SECURE_COOKIES ? '; Secure' : ''}`;
  const setSessionCookie = (/** @type {any} */ res, /** @type {string} */ token) =>
    res.setHeader('Set-Cookie', `${AUTH_COOKIE}=${token}; ${cookieTail}; Max-Age=${Math.floor(SESSION_TTL_MS / 1000)}`);
  const clearSessionCookie = (/** @type {any} */ res) =>
    res.setHeader('Set-Cookie', `${AUTH_COOKIE}=; ${cookieTail}; Max-Age=0`);
  ...
  // Email the 6-digit code. Priority: SMTP (Gmail app-password) → Resend → console fallback (so the
  // flow stays testable with no email service). Lazily build one SMTP transport.
  /** @type {any} */
  let mailer = null;
  const getMailer = async () => {
    if (mailer) return mailer;
    // @ts-ignore — nodemailer ships no bundled types; runtime-only dependency.
  ...
  const sendCode = async (/** @type {string} */ email, /** @type {string} */ code) => {
    // Echo the code to the console in dev (or when no email service exists — otherwise codes
    // would be unobtainable). Production with working email keeps codes out of the logs.
    if (LOG_CODES || !HAS_EMAIL_SERVICE) {
    ...
    if (SMTP_HOST && SMTP_USER && SMTP_PASS) {
      try {
        ...
        console.error('[eurail-auth] SMTP send failed:', /** @type {any} */ (err)?.message || err);
        console.log(`\x1b[35m[eurail-auth] CODE\x1b[0m for ${email}: ${code}  (SMTP failed — console fallback)`);
      }
      return;
    }
    if (EMAIL_API_KEY) {
  ...
  // One JSON file per conversation under <sessionsDir>/<uid>/<sessionId>.json:
  // { id, title, createdAt, updatedAt, turns: { role, content, at }[] }.
  ...
  // `uid` is always emailHash(authedEmail) — never client-supplied — so it can't reach another
  // user's files, and it's hex (no `.`/`/`) so it can't escape the sessions root.
  const userDir = (/** @type {string} */ uid) => new URL(`${uid}/`, sessionsDir);
  const sessionFileUrl = (/** @type {string} */ uid, /** @type {string} */ sid) =>
    new URL(`${sid}.json`, userDir(uid));
  const TITLE_MAX = 80;
  ...
  // corpus JSON files injected at <<CORPUS_JSON>>. Read lazily at first use.
  /** @type {string | null} */
  ...
  const buildSystemPrompt = () => {
    ...
    try {
      template = readFileSync(new URL('onboarder.system.md', eurailDir), 'utf8');
    } catch {
    ...
    for (const name of GROUND_FILES) {
      try {
        corpus[name] = JSON.parse(readFileSync(new URL(`corpus/${name}.json`, eurailDir), 'utf8'));
      } catch {
  ...
  // verifier (verifyDiagram) then drops every figure it can't ground before we emit the ```diagram
  // block. Everything below is best-effort: a load/parse/HTTP failure means no picture, never a
  // broken reply.
  ...
  const loadEvidence = () => {
    ...
    try {
      const j = JSON.parse(readFileSync(new URL('corpus/raw/evidence.json', eurailDir), 'utf8'));
  ...
  const loadCartographerPrompt = () => {
    ...
    try {
      txt = readFileSync(new URL('cartographer.system.md', eurailDir), 'utf8');
    } catch {
  ...
  const runCartographer = async (/** @type {string} */ intent) => {
    ...
    if (!brief || !LLM_API_KEY) return null; // no key → can't author → no picture
  ...
  // The deterministic verifier ships with the corpus files (src/eurail/chat/diagram/verifier.mjs).
  // Resolve it lazily + tolerantly off eurailDir: a missing verifier means no diagram (the grounding
  // guarantee is code-enforced — we NEVER render an unverified spec; no verifier, no render).
  /** @type {((spec: any, ctx: any) => any) | null | undefined} */
  let verifyDiagramFn;
  const getVerifier = async () => {
    if (verifyDiagramFn !== undefined) return verifyDiagramFn;
    try {
      const mod = await import(new URL('chat/diagram/verifier.mjs', eurailDir).href);
      const fn = mod?.verifyDiagram;
      verifyDiagramFn = typeof fn === 'function' ? fn : null;
    } catch {
      verifyDiagramFn = null; // module not present — fail closed (no verifier → no diagram)
    }
    return verifyDiagramFn;
  };

  // End-to-end: intent → cartographer → verifier → a verified spec (or null). Wrapped so any
  // failure in the chain yields null — the caller then simply emits no diagram block.
  ...
  // ════════════════════════════════════════════════════════════════════════════════════
  // The handlers — mounted in this order; the chat handler next()s non-POSTs so the more
  // specific GET /sessions route (registered after) still matches, exactly like connect.
  // ════════════════════════════════════════════════════════════════════════════════════

  use('/eurail/api/chat', async (req, res, next) => {
    ...
    // Log the question on stdout (handy when watching a session); truncated.
    ...
    res.setHeader('x-accel-buffering', 'no'); // nginx: do not buffer this stream

    // Terminator written exactly once, by whichever path finishes first.
    let ended = false;
    /** @param {string} [tail] trusted bridge note written before the terminator */
    const finish = (tail) => {
      if (ended) return;
      ended = true;
    ...
    try {
      ...
      finish(
        ctrl.signal.aborted
          ...
          : "I couldn't reach the onboarder model — check the configured endpoint (EURAIL_ONBOARDER_URL), then try again.",
      );
  ...
  // Same-origin; the mount strips the prefix, so req.url here is '' / '/' (list) or '/<id>'.
  use('/eurail/api/chat/sessions', async (req, res, next) => {
  ...
  // ── auth endpoints (@eurail.com email gate) ─────────────────────────────────
  //   POST /eurail/api/auth/request {email}      → email a 6-digit code
  ...
  use('/eurail/api/auth', async (req, res, next) => {
    ...
    if (sub === '/request' && req.method === 'POST') {
      ...
      // addresses can't grow the store unbounded (memory-DoS hardening).
