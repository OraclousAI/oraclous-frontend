// Published-agent hooks (developer surface). List is org-scoped (member); publish is admin. There
// is no display-once secret here — an agent's "invoke key" is a separate integration key bound to
// its slug, so this surface is plain CRUD (publish + list; unpublish awaits oraclous-backend #280).
